...
import { Video, Award, Clock, Play, Plus, Link as LinkIcon, Copy, Check, FileText, BarChart2 } from 'lucide-react';
...
export function Dashboard({ onJoinMeeting }: DashboardProps) {
  ...
  const activeMeetings = meetings.filter(meet => meet.status !== 'Completed');
  const completedMeetings = meetings.filter(meet => meet.status === 'Completed');

  const totalDurationSeconds = completedMeetings.reduce((sum, m) => sum + (m.attendance_duration || 0), 0);
  const totalDurationStr = totalDurationSeconds > 0 
  ...
  return (
    <div className="app-container">
      ...
      {/* Tabs Menu */}
      <div style={{ display: 'flex', gap: '16px', marginBottom: '24px', borderBottom: '1px solid var(--glass-border)', paddingBottom: '8px' }}>
        <button 
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '32px', alignItems: 'start' }}>
        {/* Left Column: Meetings or Logs List */}
        <div>
          {activeTab === 'meetings' ? (
            <>
            ...
            <>
              <h2 style={{ fontSize: '1.5rem', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <FileText className="text-primary" /> Completed Interview Logs
              </h2>
              
              {loading ? (
                <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-secondary)' }}>Loading completed logs...</div>
              ) : error ? (
                <div className="glass-card" style={{ borderLeft: '4px solid var(--danger)', padding: '20px' }}>
                  <p style={{ color: 'var(--danger)', fontWeight: 600, marginBottom: '8px' }}>Connection Error</p>
                  <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>{error}</p>
                ...
                <div className="glass-card" style={{ textAlign: 'center', padding: '40px' }}>
                  <p style={{ color: 'var(--text-secondary)' }}>No completed interview sessions found.</p>
                </div>
              ) : (
                <div className="dashboard-grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))' }}>
                    ...
                    <div key={meet.id} className="glass-card" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'between', minHeight: '230px' }}>
                      <div>
                        ...
                        <div style={{ margin: '8px 0', fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                          <strong>Duration:</strong> {Math.floor(meet.attendance_duration / 60)}m {meet.attendance_duration % 60}s
                        </div>
                      </div>
                      ...
                      <div style={{ marginTop: 'auto', paddingTop: '16px', borderTop: '1px solid rgba(255, 255, 255, 0.05)', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        {meet.pdf_url && (
                          <a 
                            href={`${API_URL}${meet.pdf_url}`} 
                            target="_blank" 
                            rel="noopener noreferrer" 
                            className="btn-primary" 
                            style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', textDecoration: 'none', padding: '10px 0', fontSize: '0.9rem' }}
                          >
                            <FileText size={14} /> View Transcript PDF
                          </a>
                        )}
                        {meet.recording_url && (
                          <a 
                            ...
                            style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', textDecoration: 'none', padding: '10px 0', fontSize: '0.9rem' }}
                          >
                            <Play size={14} /> Play Video Recording
                          </a>
                        )}
                      </div>
                    </div>
        ...
        {/* Right Column: Generate Form or Summary Analytics */}
        <div>
          {activeTab === 'meetings' ? (
          ...
          <div className="glass-panel" style={{ padding: '24px', position: 'sticky', top: '40px' }}>
            <h3 style={{ fontSize: '1.2rem', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <BarChart2 size={20} className="text-primary" /> Log Summary
            </h3>
            <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '20px' }}>
              Analytics overview of all completed mock interview sessions.
            </p>
            
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div className="glass-card" style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Interviews Conducted</span>
                <span style={{ fontSize: '1.75rem', fontWeight: 700, color: 'var(--primary)' }}>{completedMeetings.length}</span>
              </div>
              
              <div className="glass-card" style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Total Call Duration</span>
                <span style={{ fontSize: '1.75rem', fontWeight: 700, color: 'var(--success)' }}>{totalDurationStr}</span>
              </div>
            </div>
          </div>
        )}
